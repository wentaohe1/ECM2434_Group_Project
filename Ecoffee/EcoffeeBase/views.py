...
def log_visit(request):
    ...
    user_shop, create_new = UserShop.objects.get_or_create(
        user=custom_user, shop_id=shop, defaults={'visit_amounts': 0})
    user_shop.visit_amounts += 1
    user_shop.save()

    for badge in Badge.objects.all():
        if custom_user.cups_saved >= badge.coffee_until_earned:
            user_badge, create_new = UserBadge.objects.get_or_create(
                user=custom_user, badge_id=badge)
