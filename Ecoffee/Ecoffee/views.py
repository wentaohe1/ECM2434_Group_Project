...
def dashboard_view(request):
    if request.user.is_authenticated:
        ...
        if next_badge!=None and user_badge!='':
            coffees_to_next_badge=int(next_badge.coffee_until_earned)-int(request_user.cups_saved)
            progress=100-round((coffees_to_next_badge/int(next_badge.coffee_until_earned)*100))
        else:
